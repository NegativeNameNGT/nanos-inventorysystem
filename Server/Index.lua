...
function Player:AddItemToPlayer(item_rowname, isload, slot, quantity, uuid, metadata, save)
    ...
    if player:GetValue("inventory") == nil then
        player:SetValue("inventory", {})
    end
    if DoesItemRowExists(item_rowname) == false then
        return
    end
    item_data = items[item_rowname]
    if uuid == nil then
        uuid = generateUUID():lower():gsub("%-", "")
    end
    if slot == nil then
        slot = GetEmptySlot(player) - 1
    end
    ...
    if save == nil then
        ...
        Events.CallRemote(
            ...
            uuid,
            slot,
            GetItemIcon(item_data),
            item_data["metadata"]["rarity"],
            quantity,
            IsItemStackable(item_data)
        )
    else
        return {
            ["uuid"] = uuid,
